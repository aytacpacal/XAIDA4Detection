...
class ClassificationVisualizer2D(ClassificationVisualizerAbstract):
    ...
    def __plot_per_class_detection(self, output, sample_name):
        ...
        for num_class in self.data_classes:
            ...
            misc.plot_spatial_signal(self.save_path+'/spatial_visualization/'+str(num_class)+'_'+sample_name, output[num_class-1], mask, self.coordinates)
    ...
    def global_operations(self):
        ...
        if self.config['data']['time_aggregation']:
            for num_class in self.data_classes:
                misc.plot_temporal_results(self.save_path+'/'+str(num_class)+'_', self.temporal_plot_xticks_labels,
