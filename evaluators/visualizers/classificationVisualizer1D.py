...
class ClassificationVisualizer1D(ClassificationVisualizerAbstract):
    ...
    def __init__(self, config, model, dataset):
        ...
        self.cm = ConfusionMatrix(task= confusion_matrix_task, num_classes = len(self.data_classes)+1, normalize='true')
    ...
    def __plot_per_class_detection(self, output, labels, sample_name):
        ...
        ax.plot(labels/torch.max(labels), c = cycle[np.unique(labels)[-1]-1], label = 'Event GT', linestyle='dashed', linewidth=3)
        ax.fill_between(np.arange(len(labels/torch.max(labels))), labels/torch.max(labels), 0, color=cycle[np.unique(labels)[-1]-1], alpha=0.4)
        plt.ylim(bottom=-0.1)
        legend = ['Non storm', 'Convective storm', 'Extra-tropical storm', 'Tropical cyclone']
        for num_class in range(1, self.num_classes):
            ax.plot(output[num_class], label = 'P(t) class ' + legend[num_class], linewidth=3)
    ...
    def global_operations(self):
        ...
        if self.config['data']['time_aggregation']:
            misc.plot_temporal_results(self.save_path+'/temporal_visualization/', self.temporal_plot_xticks_labels, self.test_labels, self.test_outputs)
